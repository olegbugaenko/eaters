...
import resourcesEn from "@/localization/en/resources.json";
import spellsEn from "@/localization/en/spells.json";
...
import resourcesUa from "@/localization/ua/resources.json";
import spellsUa from "@/localization/ua/spells.json";
...
import resourcesDe from "@/localization/de/resources.json";
import spellsDe from "@/localization/de/spells.json";
...
import resourcesPl from "@/localization/pl/resources.json";
import spellsPl from "@/localization/pl/spells.json";
...
import resourcesRu from "@/localization/ru/resources.json";
import spellsRu from "@/localization/ru/spells.json";
import {
  DEFAULT_LANGUAGE,
...
type LocalizationBundle = {
  ...
  readonly resources: Readonly<Record<string, { readonly name?: string }>>;
  readonly spells: Readonly<Record<string, { readonly name?: string; readonly description?: string }>>;
};
...
export class LocalizationService implements LocalizationServiceUiApi {
  ...
  public getResourceName(resourceId: ResourceId, fallback: string): string {
    const localized = bundles[this.language].resources[resourceId]?.name;
    return localized ?? fallback;
  }

  public getSpellText(spellId: SpellId, fallback: LocalizedSpellText): LocalizedSpellText {
    const localized = bundles[this.language].spells[spellId];
    return {
      name: localized?.name ?? fallback.name,
      description: localized?.description ?? fallback.description,
    };
