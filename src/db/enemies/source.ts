...
export const ENEMIES_DB_SOURCE: Record<EnemyType, EnemyConfig> = {
  basicEnemy: {
    name: "Basic Enemy",
    renderer: {
      kind: "polygon",
...
export const ENEMY_TYPES = Object.keys(ENEMIES_DB_SOURCE) as EnemyType[];
...
export const getEnemyConfig = (type: EnemyType): EnemyConfig => {
  const config = ENEMIES_DB_SOURCE[type];
  if (!config) {
