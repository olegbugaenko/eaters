...
describe("PlayerUnitAbilities sound effects", () => {
  ...
  test("throttles repeated sound for the same ability trigger", () => {
    const audioCalls: string[] = [];
    const audio: AbilitySoundPlayer = {
      playSoundEffect: (url) => audioCalls.push(url),
    };

    const abilities = createAbilities([], { audio });
    const abilityAudio = abilities as unknown as {
      playAbilitySound: (trigger: "heal" | "frenzy" | "fireball") => void;
    };
    ...
    try {
      abilityAudio.playAbilitySound("heal");
      abilityAudio.playAbilitySound("heal");
      now += 399;
      abilityAudio.playAbilitySound("heal");
      now += 1;
    ...
    assert.deepStrictEqual(audioCalls, [
      "/audio/sounds/brick_effects/heal.mp3",
      "/audio/sounds/brick_effects/heal.mp3",
    ]);
  });

  test("allows different ability sounds to overlap", () => {
    const audioCalls: string[] = [];
    const audio: AbilitySoundPlayer = {
      playSoundEffect: (url) => audioCalls.push(url),
    };
    ...
    try {
      abilityAudio.playAbilitySound("heal");
      abilityAudio.playAbilitySound("frenzy");
    } finally {
      Date.now = originalNow;
    }
    ...
    assert.deepStrictEqual(audioCalls, [
      "/audio/sounds/brick_effects/heal.mp3",
      "/audio/sounds/brick_effects/buff.mp3",
    ]);
  });
});
