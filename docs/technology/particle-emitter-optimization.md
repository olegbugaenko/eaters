...
- Для `ringTrail` у проджектайлів можливий `offset`, який масштабується на радіус снаряда та обертається за напрямком руху перед спавном кільця.
- `fadeInMs` дозволяє плавно вводити альфу частинок і кілець протягом заданого часу, без додаткових CPU-алокацій (множник застосовується в шейдерах).
