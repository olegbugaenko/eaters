...
## Логіка (`src/logic`)
- `core/` — каркас застосунку: `Application.ts` збирає сервіси та модулі, `DataBridge.ts` прокидає реактивні дані в UI, `ServiceContainer.ts` зберігає сінглтони, `ModuleRegistry.ts` керує реєстром модулів.
  - `core/factories/` — `StateFactory` та його наслідники для уніфікованого створення станів модулів.
- `config/` — налаштування логіки, зокрема перелік активних модулів через `config/modules.ts`.
...
- `definitions/` — декларативне збирання модулів і сервісів, включно з `definitions/modules/registry.ts`.
