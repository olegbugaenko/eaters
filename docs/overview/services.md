...
Сервіс, що збирає дерево потенційних відкриттів, тримає статуси «відкрито/переглянуто» та публікує в DataBridge зведення по нових елементах. Модулі реєструють шляхи через `registerUnlock`, а UI відмічає перегляд через `uiApi.newUnlocks.markViewed()`.
