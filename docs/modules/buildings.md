...
Модуль використовує `BuildingStateFactory` (наслідник `StateFactory`) для створення станів будівель:

- **`create`**: обчислює доступність будівлі (на основі умов розблокування), вартість наступного апгрейду, бонуси та максимальний рівень
- **`transform`**: не використовується (немає side effects, стан використовується тільки для UI)

Метод `computeWorkshopState` використовує `stateFactory.create` для створення станів будівель.
